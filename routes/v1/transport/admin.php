...
$router->group(['middleware' => 'auth:admin'], function ($app) {
    ...
    $app->get('/getvehicletype', 'V1\Transport\Admin\VehicleController@getvehicletype');
		...
		$app->post('/rental/rideprice', 'V1\Transport\Admin\VehicleController@rentalrideprice');
		$app->post('/outstation/rideprice', 'V1\Transport\Admin\VehicleController@outstationrideprice');
		
		$app->get('/rental/package_delete/{id}', ['middleware' => 'demo', 'uses' => 'V1\Transport\Admin\VehicleController@rentalPackageDelete']);

		$app->get('/rideprice/{ride_delivery_vehicle_id}/{city_id}/{service_type}', 'V1\Transport\Admin\VehicleController@getRidePrice');
		...
		$app->get('findprovider/{id}', 'V1\Transport\User\RideController@findprovider');

		$app->patch('/requestscheduleupdate/{id}', ['middleware' => 'demo', 'uses' => 'V1\Transport\User\RideController@scheduleupdate']);
