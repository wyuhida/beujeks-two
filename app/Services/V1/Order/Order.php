...
class Order {
    ...
    */
    ...
    public function callTransaction($store_order_id){  
        ...
        if($StoreOrder->paid==1){
            ...
            if($StoreOrder->order_type=="DELIVERY")
            {
                ...
                $transation['id']=$StoreOrder->store_id;
                $transation['amount']=$paymentsStore->commision_amount;
                $this->shopDebitCommission($transation);
            }             
    ...
    protected function shopDebitCommission($request){       
  
        $ad_det_amt= -1 * abs($request['amount']);                            
        $request['transaction_desc']='Admin Commission sent';
