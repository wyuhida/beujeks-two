...
class UserServices {
	...
	public function createRequest($Providers, $newRequest, $type) {
		...
		$user_request->service_type = isset($newRequest->service_type) ? $newRequest->service_type:'';
		$user_request->save();
				...
				if($this->settings->transport->manual_request == 0){
					...
					foreach ($Providers as $key => $Provider) {
						...
						//if($existingRequest == null) {
							...
							if($existingRequest) {
								$diff = (Carbon::now())->diffInSeconds($existingRequest->added_at);
								...
								if($diff >= $Timeout) {
									$Filter->added_at = Carbon::now();
								} else {
									$Filter->added_at = (Carbon::now())->addSeconds(   (Carbon::now())->diffInSeconds($existingRequest->added_at)   );
								}
