...
trait HasPushSubscriptions
{
     ...
     */
    public function updatePushSubscription($endpoint, $key = null, $token = null, $contentEncoding = null, $type = null)
    {
        $model = config('webpush.model');
        if($type && $type == 'shop') $model = '\App\Models\Order\PushSubscription';
        $subscription = app($model)->findByEndpoint($endpoint);
