...
use App\Models\Transport\RentalRideCityPrice;
...
class VehicleController extends Controller
{
    ...
    public function gettaxiprice($id)
    {
       ...
       if($admin_service){
       $cityList = CompanyCountry::with('country','companyCountryCities','companyCountryStates')->where('company_id',Auth::user()->company_id)->where('status',1)->get();
        ...
        $countries = $cityList->map(function ($response) {
            ...
            $state = [];
            foreach ($response->companyCountryStates as $value) {
                $state[] = $value->state;
            }
...
//return $countries;
       ...
       /* $this->validate($request, [
        ...
        try{
            ...
            foreach ($request->geofence_id as $geofence) {
                ...
                if(!empty($request->input($geofence.'_city_limit')) || !empty($request->city_limit))
                {
                    if($request->distance_unit=='Kms')
                    {
                        $ridePrice->city_limit = round(($request->input($geofence.'_city_limit') ? $request->input($geofence.'_city_limit'): $request->city_limit)*1000,1);
                    }
                    else{
                        $ridePrice->city_limit = $request->input($geofence.'_city_limit') ? $request->input($geofence.'_city_limit'): $request->city_limit;
                    }
                }
                else
                {
                    $ridePrice->city_limit=0;
                }
    ...
    public function outstationrideprice(Request $request)
    {

         $this->validate($request, [
            'state_id' => 'required',
            'base_distance' => 'required',
            'oneway_distance_price' => 'required|numeric',
            'driver_allowance' => 'required|numeric',
            'night_allowance' => 'required|numeric',
            'extra_distance_price' => 'required|numeric',
            'extra_time_price' => 'required|numeric',
            'outstation_fleet_commission' => 'required|numeric',
            'roundtrip_distance_price' => 'sometimes|nullable|numeric',
            'outstation_tax' => 'sometimes|nullable|numeric',
            'outstation_commission' => 'sometimes|nullable|numeric',
        ...
        try{
                ...
                $ridePrice = OutstationRideStatePrice::where([['company_id',Auth::user()->company_id],['ride_delivery_vehicle_id',$request->ride_delivery_vehicle_id],['state_id',$request->state_id]])->first();
                if($ridePrice == null){
                    $ridePrice = new OutstationRideStatePrice;
                }
                $ridePrice->company_id = Auth::user()->company_id;
                $ridePrice->state_id = $request->state_id;  
                $ridePrice->ride_delivery_vehicle_id = $request->ride_delivery_vehicle_id;

                $ridePrice->distance=$request->base_distance;
                $ridePrice->oneway_price=$request->oneway_distance_price;
                $ridePrice->roundtrip_price=$request->roundtrip_distance_price;

                $ridePrice->driver_allowance=$request->driver_allowance;
                $ridePrice->night_time_allowance=$request->night_allowance;
                $ridePrice->per_hour_price=$request->extra_time_price;
                $ridePrice->per_km_price=$request->extra_distance_price;

                $ridePrice->commission=$request->outstation_commission;
                $ridePrice->fleet_commission=$request->outstation_fleet_commission;
                $ridePrice->tax =$request->outstation_tax;
                $ridePrice->save();
    ...
    public function rentalrideprice(Request $request)
    {

         $this->validate($request, [
            'city_id' => 'required',
            'packages' => 'required',
            'rental_fixed' => 'required|numeric',
            'rental_distance' => 'sometimes|nullable|numeric',
            'rental_tax' => 'sometimes|nullable|numeric',
            'rental_commission' => 'sometimes|nullable|numeric',

        ]);


        try{
                $ridePrice = RideCityPrice::with('rentalPackages')->where('service_type','RENTAL')->where([['company_id',Auth::user()->company_id],
                                         ['ride_delivery_vehicle_id',$request->ride_delivery_vehicle_id],
                                         ['city_id',$request->city_id]])->first();
                if($ridePrice == null){
                    $ridePrice = new RideCityPrice;
                }
                $ridePrice->company_id = Auth::user()->company_id;
                $ridePrice->city_id = $request->city_id;  
                $ridePrice->service_type = "RENTAL";  
                $ridePrice->ride_delivery_vehicle_id = $request->ride_delivery_vehicle_id;
                $ridePrice->rental_hour_price=$request->rental_fixed;
                $ridePrice->rental_km_price=$request->rental_distance;
                $ridePrice->commission=$request->rental_commission;
                $ridePrice->fleet_commission=$request->rental_fleet_commission;
                $ridePrice->tax =$request->rental_tax;
                $ridePrice->peak_commission=$request->rental_peak_commission;
                $ridePrice->save();
                ...
                if($packages){
                    foreach ($packages as $key => $package_list) {
                        ...
                        if(!empty($package_list['package_hour']) && !empty($package_list['package_distance'])) {
                            if(!empty($package_list['id'])){
                                $RentalPackage = RentalPackage::findOrFail($package_list['id']);
                            }else{
                                $RentalPackage = new RentalPackage;
                            }  
                            $RentalPackage->ride_city_price_id =$ridePrice->id;
                            $RentalPackage->hour =$package_list['package_hour'];
                            $RentalPackage->km =$package_list['package_distance'];
                            $RentalPackage->price =($package_list['package_price'] !='')?$package_list['package_price']:'0.00';
                            $RentalPackage->save();
                        } 
                    }
                }
    ...
    public function rentalPackageDelete($id)
    {
        $RentalPackage = RentalPackage::destroy($id);
         return Helper::getResponse(['message' => trans('admin.user_msgs.user_delete')]);
    }
    public function getRidePrice($ride_delivery_vehicle_id,$city_id,$service_type)
    {
        
        if($service_type== "OUTSTATION")
        {
            $rideCityPrice = OutstationRideStatePrice::where([['company_id',Auth::user()->company_id],
                                         ['ride_delivery_vehicle_id',$ride_delivery_vehicle_id],
                                         ['state_id',$city_id]])->first();
            $rideCityPriceList = OutstationRideStatePrice::where([['company_id',Auth::user()->company_id],
                                         ['ride_delivery_vehicle_id',$ride_delivery_vehicle_id],
                                         ['state_id',$city_id]])->get();
        }
        else
        {
            $rideCityPrice = RideCityPrice::with('rentalPackages')->where('service_type',$service_type)->where([['company_id',Auth::user()->company_id],
                                         ['ride_delivery_vehicle_id',$ride_delivery_vehicle_id],
                                         ['city_id',$city_id]])->where('pricing_differs', 0)->first();
            $rideCityPriceList = RideCityPrice::with('rentalPackages')->where('service_type',$service_type)->where([['company_id',Auth::user()->company_id],
                                         ['ride_delivery_vehicle_id',$ride_delivery_vehicle_id],
                                         ['city_id',$city_id]])->get();
        }
