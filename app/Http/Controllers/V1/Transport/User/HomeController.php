...
use Actions;
    ...
    public function trips(Request $request) {
        try{
             ...
             $userrequest=RideRequest::select('id', 'booking_id', 'assigned_at', 's_address', 'd_address','provider_id','user_id','timezone','ride_delivery_id', 'status', 'user_rated', 'provider_rated','payment_mode', 'provider_vehicle_id','created_at','schedule_at','service_type');
	...
	public function gettripdetails(Request $request,$id) {
		try{
			...
			$jsonResponse = [];
			$jsonResponse['type'] ='transport';
			$userrequest = RideRequest::with(['provider','rental_package','payment','payment.promoCode','service_type','ride','provider_vehicle','dispute'=> function($query){  
			...
			if($data->payment)
			{
				...
				if($data->service_type=="RENTAL")
					$data->payment->base_fare_text = 'Base Fare for ('.$data->rental_package->hour .'hr '.$data->base_distance .' '.$data->unit.')';
				else
	...
	public function get_ride_lost_item(Request $request,$id) {
		$ride_lost_item = RideLostItem::where('company_id',Auth::guard('user')->user()->company_id)
	     ...
	     if($ride_lost_item && get_object_vars($ride_lost_item) > 0){
