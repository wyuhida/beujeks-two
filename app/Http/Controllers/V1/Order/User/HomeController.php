...
class HomeController extends Controller
{
    ...
    public function show_addons(Request $request,$id){
    	$item_addons =$Item= StoreItem::with(['itemsaddon.addon'=>function($query) use ($request){
            $query->where('addon_status',1);
        },'itemcartaddon'])->where('company_id',$this->company_id)->select('id','item_name','item_price','item_discount_type','item_discount')->find($id);
    ...
    public function checkout(Request $request){
        ...
        if($request->payment_mode=='MIDTRANS'){ 
            $payable = $cart['payable'];
            if($payable!=0){
                $payment = $this->orderpayment($payable,$request);
                return $payment;
            }
        }


        if($request->payment_mode=='MIDTRANS_SUCCESS'){ 
            $payable = $cart['payable'];
            if($payable!=0){
                $payment_id = $request->order_id;

                $request['payment_mode'] = 'MIDTRANS';
                if($payment_id=='failed'){
    ...
    public function requestStatementHistory(Request $request)
	{
		try {
            ...
            if($request->has('page') && $request->page == 'all') {
                $dataval = $orderRequests->whereIn('status',$history_status)->get();
            } else {
	...
	{
		try {
			$data = StoreOrder::with('user', 'provider','orderInvoice','reason','rating')->findOrFail($id);
            return Helper::getResponse(['data' => $data]);
        } catch (\Throwable $e) {
            return Helper::getResponse(['status' => 404,'message' => trans('admin.something_wrong'), 'error' => $e->getMessage()]);
        }
        ...
        if($request->has('q')){
            ...
            if($search_type=='store'){ 
                ...
                $data->map(function ($item) {
                    ...
                    $item['category'] = $item->categories->store_category_name;
                    }else{
                    $item['category'] = null;  
                    }
                    unset($item->store);
