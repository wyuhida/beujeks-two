...
class HomeController extends Controller
{
		...
		try{
			if($request->admin_service != 'SERVICE'){
				...
				if($request->has('is_rental')) {
					$providervehicle->is_rental=$request->is_rental;
				}else{
					$providervehicle->is_rental=0;    
				}

				if($request->has('is_outstation')) {
					$providervehicle->is_outstation=$request->is_outstation;
				}else{
					$providervehicle->is_outstation=0;    
				}
	...
	public function servicesRide_status(Request $request,$id)
	{
		if(!empty($id))
		{
			$provider_service = ProviderVehicle::where('admin_service','TRANSPORT')->where('id',$id)->first();
			
		    if ($request->is_rental == 1) {
		    	$provider_service->is_rental= 1;   
		    }else{
		    	$provider_service->is_rental= 0;
		    }

		   	if ($request->is_outstation == 1) {
			  $provider_service->is_outstation=1;   
		    }
		    else{
		    	$provider_service->is_outstation= 0;
		    }
	...
	public function editvechile(Request $request)
	{
		...
		try{
			if($request->admin_service !='SERVICE'){
			    ...
			    if($request->has('is_rental')) {
					$providervehicle->is_rental=1;
				}else{
					$providervehicle->is_rental=0;    
				}

				if($request->has('is_outstation')) {
					$providervehicle->is_outstation=1;
				}else{
					$providervehicle->is_outstation=0;    
				}
