...
class ZoneController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try{
            $datum = Zone::with('city')->where('company_id',Auth::user()->company_id)->get();
            foreach($datum as $avail)
            {
    
                $available_name = strtoupper(str_replace(' ', '', $avail->name));
                $avail_city_name = strtoupper($avail->city->id);
                if(($available_name === strtoupper(str_replace(' ', '', $request->name))) && ($avail_city_name === strtoupper($request->city_id)))
                {
                    return Helper::getResponse(['status' => 422, 'message' => ("$request->name Already Exists in the City " .$avail->city->city_name) ]);
                }
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            foreach($datum as $avail)
            {
                ...
                $available_name = strtoupper(str_replace(' ', '', $avail->name));
                $avail_city_name = strtoupper($avail->city->id);
                if(($available_name === strtoupper(str_replace(' ', '', $request->name))) && ($avail_city_name === strtoupper($request->city_id)) && ($avail->id != $request->id) )
                {
                    return Helper::getResponse(['status' => 422, 'message' => ("$request->name Already Exists in the City " .$avail->city->city_name) ]);
                }
            }
