...
class CompanyCountriesController extends Controller
{
    ...
    public function cities(Request $request, $id)
    {
        if(!empty($request->id)){
            ...
            $company_cities = CompanyCity::where('company_id',\Auth::user()->company_id)->where('id','!=', $request->id)->pluck('city_id')->all();
         }else{
        ...
        $cities = City::where("state_id",$id)->whereNotIn('id',$company_cities)->get();
