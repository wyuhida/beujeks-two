...
class AdminController extends Controller
{
	...
	{    
		...
		if($setting != null) {
			...
			if($request->has('provider_select_timeout')) {
				...
				if(!$request->has('ride_enable')) {
					$request->request->add(['ride_enable' => '0']);
				}
				if(!$request->has('outstation_enable')) {
					$request->request->add(['outstation_enable' => '0']);
				}
				if(!$request->has('rental_enable')) {
					$request->request->add(['rental_enable' => '0']);
				}
				if(!$request->has('rental_package_change')) {
					$request->request->add(['rental_package_change' => '0']);
				}
			...
			if($request->has('delivery_provider_search_radius')) {
			...
			foreach($request->only(['user_select_timeout', 'provider_select_timeout', 'provider_search_radius','outstation_provider_search_radius','credit_ride_limit', 'manual_request', 'broadcast_request', 'ride_otp', 'booking_prefix','rental_package_change','ride_enable','outstation_enable','rental_enable']) as $key => $req) {
			...
			foreach($request->only(['delivery_user_select_timeout', 'delivery_provider_select_timeout', 'delivery_provider_search_radius', 'delivery_manual_request', 'delivery_otp', 'delivery_booking_prefix','distance_unit','delivery_unit_measurement']) as $key => $req) {
			...
			if($request->hasFile('user_pem')) {
				$user_pem = Helper::upload_file($request->file('user_pem'), 'apns', 'user.p8');
				$data['site']['user_pem'] = $user_pem;
			}

			if($request->hasFile('provider_pem')) {
				$provider_pem = Helper::upload_file($request->file('provider_pem'), 'apns', 'provider.p8');
				$data['site']['provider_pem'] = $provider_pem;
			}

			if($request->hasFile('shop_pem')) {
				$shop_pem = Helper::upload_file($request->file('shop_pem'), 'apns', 'shop.p8');
				$data['site']['shop_pem'] = $shop_pem;
			}

			if($request->hasFile('ios_push_pk')) {
				$ios_push_pk = Helper::upload_file($request->file('ios_push_pk'), 'apns', 'private_key.p8');
				$data['site']['ios_push_pk'] = $ios_push_pk;
			}

			$setting->settings_data = json_encode($data);
			$setting->save();

			if($request->adminservice && count($request->adminservice) > 0 &&$request->has('adminservice')){
	...
	public function dashboarddata($id)
	{
	  try{
	  	  ...
	  	  $data['admin_type']=Auth::user()->type;
		  ...
		  $data['fleetdata']=Admin::where('country_id', $id)->where('type','FLEET')->where('company_id',\Auth::user()->company_id)->count();
			...
			$data['provider_debit'] = Provider::select(DB::raw('SUM(CASE WHEN wallet_balance<0 THEN wallet_balance ELSE 0 END) as total_debit'))->when(Auth::user()->type == 'FLEET', function ($q) {return $q->where('admin_id', Auth::user()->id);})->where('country_id', $id)->where('company_id',\Auth::user()->company_id)->get()->toArray();
			$data['provider_credit'] = Provider::select(DB::raw('SUM(CASE WHEN wallet_balance>=0 THEN wallet_balance ELSE 0 END) as total_credit'))->when(Auth::user()->type == 'FLEET', function ($q) {return $q->where('admin_id', Auth::user()->id);})->where('country_id', $id)->where('company_id',\Auth::user()->company_id)->get()->toArray();
