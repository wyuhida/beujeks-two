...
class SocialLoginController extends Controller
{
	...
	try{	
		...
		if($provider == 'user')  {
			...
			if($user){
				...
				$newUser = User::with('state')->find($user->id);
	...
	public function push(Request $request, $type) {
		...
		if($type == 'IOS') {
			...
			$pem = app()->basePath('storage/app/public/'.$request->company_id.'/apns' ).'/private_key.p8';

			if(file_exists($pem)) {

	             $options = [
				    'key_id' => $settings->settings_data->site->ios_push_key_id, // The Key ID obtained from Apple developer account
				    'team_id' => $settings->settings_data->site->ios_push_team_id, // The Team ID obtained from Apple developer account
				    'app_bundle_id' => $settings->settings_data->site->ios_push_user_bundle, // The bundle ID for app obtained from Apple developer account
                ...
                $alert = $alert->setBody('Hi Test Push');
                ...
                $notification = new Notification($payload, $request->token);
                ...
                $client = new Client($authProvider, $settings->settings_data->site->environment == "production" ? true : false);
			...
			if($settings->settings_data->site->android_push_key != "") {
	            ...
	            $push->setMessage([
	                ...
	                ->setApiKey($settings->settings_data->site->android_push_key)
	                ->setDevicesToken($request->token)
	                ->send()
	                ->getFeedback();
