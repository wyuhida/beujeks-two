...
class Helper {
	...
	public static function siteRegisterMail($user){
		...
		config([
			'mail.driver' =>  trim($settings->site->mail_driver),
			'mail.host' =>  trim($settings->site->mail_host),
	        'mail.port' =>  trim($settings->site->mail_port),
	        'mail.username' =>  trim($settings->site->mail_username),
	        'mail.password' =>  trim($settings->site->mail_password) ,
	        'mail.encryption' =>  trim($settings->site->mail_encryption)
	    ]);

	    if($settings->site->mail_driver == 'MAILGUN'){
	    	config([
				'mail.mailgun.domain' =>  trim($settings->site->mail_domain),
				'mail.mailgun.secret' =>  trim($settings->site->mail_secret)
		    ]);
		}
