...
class ScheduleCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $userRequestList = UserRequest::where('status','SCHEDULED')
                        ->where('admin_service', 'TRANSPORT')
                        ->whereNull('provider_id')
                        ->where('schedule_at','<=',Carbon::now()->addMinutes(20))
                        ->get()->toArray();
        $userOutstationRequest = UserRequest::where('service_type','OUTSTATION')
                                            ->where('status','SCHEDULED')
                                            ->where('admin_service', 'TRANSPORT')
                                            ->whereNull('provider_id')
                                            ->where('schedule_at','<=',Carbon::now()->addMinutes(120))
                                            ->get()->toArray();    
                    
        $userRequest = array_merge($userRequestList,$userOutstationRequest);  
        $userRequest = json_encode($userRequest);       
        ...
        if(!empty($userRequest)){
            ...
            foreach($userRequest as $newRequest){
                ...
                $newRequest = UserRequest::find($newRequest->id);
                if($rideRequest != null) {

                    $distance = $rideRequest->service_type == "OUTSTATION" ? (isset($transportConfig->outstation_provider_search_radius) ? $transportConfig->outstation_provider_search_radius : 40) : (isset($transportConfig->provider_search_radius) ? $transportConfig->provider_search_radius : 30);
                    ...
                    if(!empty($Providers->toArray())) {
                        ...
                        $rideData = RideRequest::with('ride','ride_type','rental_package')->where('id', $rideRequest->id)->first();
