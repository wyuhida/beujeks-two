...
class RideRequest extends BaseModel
{
    ...
    protected $appends = ['departure_time','return_time','created_time','assigned_time', 'schedule_time', 'started_time', 'finished_time','total_distance','total_days'];
    ...
    public function getTotalDistanceAttribute() {
        ...
        if($this->service_type =='RENTAL')
        {
            $total_distance = round($this->distance,1);
        }else{
    ...
    public function getFinishedTimeAttribute() {
     ...
     public function getDepartureTimeAttribute() {
        return (isset($this->attributes['departure_date'])) ? (\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $this->attributes['departure_date'], 'UTC'))->setTimezone($this->attributes['timezone'])->format(Helper::dateFormat(1)) : '' ;
        
    }

    public function getReturnTimeAttribute() {
        return (isset($this->attributes['return_date'])) ? (\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $this->attributes['return_date'], 'UTC'))->setTimezone($this->attributes['timezone'])->format(Helper::dateFormat(1)) : '' ;

    }

    public function getTotalDaysAttribute() {
        if(isset($this->attributes['return_date']))
        {
            $from = \Carbon\Carbon::createFromFormat('Y-m-d H:s:i', isset($this->attributes['departure_date']) ? $this->attributes['departure_date'] :"");
            $to = \Carbon\Carbon::createFromFormat('Y-m-d H:s:i', isset($this->attributes['return_date'])?$this->attributes['return_date']:'');
            $days = $from->diff($to)->days;
            $hours =  $from->diff($to)->format('%H');
            $minutes =  $from->diff($to)->format('%s');
            return $days.' days '.$hours .' hours '.$minutes .' minutes'; 
        }
        elseif(isset($this->attributes['departure_date'])){
            $from = \Carbon\Carbon::createFromFormat('Y-m-d H:s:i', isset($this->attributes['created_at']) ? $this->attributes['created_at'] :"");
            $to = \Carbon\Carbon::createFromFormat('Y-m-d H:s:i', isset($this->attributes['departure_date'])?$this->attributes['departure_date']:'');
            $days = $from->diff($to)->days;
            $hours =  $from->diff($to)->format('%H');
            $minutes =  $from->diff($to)->format('%s');
            return $days.' days '.$hours .' hours '.$minutes .' minutes'; 
    ...
    {
       return $this->belongsTo('App\Models\Transport\RentalPackage', 'rental_package_id');
    }
	...
	public function scopeRideRequestStatusCheck($query, $user_id, $check_status, $admin_service,$type)
	{
		return $query->where('ride_requests.user_id', $user_id)
                    ...
                    },'ride','service_type.vehicle','payment','rental_package','rating','chat']);
	}
